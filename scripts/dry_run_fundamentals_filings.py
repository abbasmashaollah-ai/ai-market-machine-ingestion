...
from app.normalization.fundamentals_filings import (
    DEFAULT_FIXTURE_RECORDS,
    RECORD_FAMILIES,
    normalize_fundamentals_filings_record,
    validate_fundamentals_filings_record,
)


def _build_summary(source_records: tuple[dict[str, object], ...]):
    normalized_records = tuple(normalize_fundamentals_filings_record(record) for record in source_records)
    invalid_records = tuple(
        {"record": record, "errors": validate_fundamentals_filings_record(record)}
        for record in normalized_records
        if validate_fundamentals_filings_record(record)
    )
...
def _emit(*, normalized_records, invalid_records, show_records: bool, show_invalid: bool) -> None:
    ...
    print(f"record_families={[record.record_family for record in normalized_records]}")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Dry-run the fundamentals/filings foundation without database writes.")
    parser.add_argument("--symbol", action="append", help="Filter to one or more symbols.")
    parser.add_argument("--record-family", action="append", help="Filter to one or more record families.")
    ...
    if args.symbol:
        ...
        records = tuple(record for record in records if record.get("symbol") in allowed)
    if args.record_family:
        allowed_families = set(args.record_family)
        records = tuple(record for record in records if record.get("record_family") in allowed_families)
