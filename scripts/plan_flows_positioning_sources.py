...
def main() -> int:
    candidates = sorted(build_flows_positioning_source_candidates(), key=lambda item: item.priority)
    ...
    print(f"target_datasets={sorted({dataset for candidate in candidates for dataset in candidate.target_datasets})}")
    for candidate in candidates:
        print(
            f"source={candidate.source_name} "
            f"status={candidate.status} "
            f"priority={candidate.priority} "
            f"target_datasets={list(candidate.target_datasets)} "
    ...
    print("next_required_step=approve_the_flows_positioning_data_contract_and_select_the_first_live_source")
    return 0
