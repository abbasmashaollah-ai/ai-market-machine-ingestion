...
def main() -> int:
    candidates = sorted(build_cross_asset_ohlcv_source_candidates(), key=lambda item: item.priority)
    ...
    print(f"target_groups={sorted({group for candidate in candidates for group in candidate.target_groups})}")
    for candidate in candidates:
        print(
            f"source={candidate.source_name} "
            f"status={candidate.status} "
            f"priority={candidate.priority} "
            f"target_groups={list(candidate.target_groups)} "
    ...
    print("next_required_step=approve_the_cross_asset_ohlcv_data_contract_and_select_the_first_live_source")
    return 0
