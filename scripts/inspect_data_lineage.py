...
def main() -> int:
    parser = argparse.ArgumentParser(description="Inspect manual data lineage safely.")
    ...
    try:
        rows = _fetch_all(
            connection,
            """
            SELECT vendor, dataset, symbol, timeframe, source_endpoint, request_params, response_status, row_count, normalization_version, quality_status, run_id, job_id, created_at
            FROM data_lineage
        ...
        for row in rows:
            print(
                ...
                f"source_endpoint={row.get('source_endpoint')} "
                f"response_status={row.get('response_status')} "
                f"row_count={row.get('row_count')} "
                f"normalization_version={row.get('normalization_version')} "
                f"quality_status={row.get('quality_status')} "
