...
def main() -> int:
    candidates = sorted(build_breadth_participation_source_candidates(), key=lambda item: item.priority)
    ...
    print(f"target_metrics={sorted({metric for candidate in candidates for metric in candidate.target_metrics})}")
    for candidate in candidates:
        print(
            f"source={candidate.source_name} "
            f"status={candidate.status} "
            f"priority={candidate.priority} "
            f"target_metrics={list(candidate.target_metrics)} "
    ...
    print("next_required_step=approve_the_breadth_participation_data_contract_and_select_the_first_live_source")
    return 0
