...
def test_preflight_pass():
    import scripts.preflight_cross_asset_ohlcv_operations as mod
    ...
    assert "expected_asset_groups_configured=True" in printed
...
def test_no_forbidden_imports():
    text = Path("scripts/preflight_cross_asset_ohlcv_operations.py").read_text(encoding="utf-8")
    lowered = text.lower()
...
def test_docs_coverage():
    preflight = Path("docs/cross_asset_ohlcv_preflight.md").read_text(encoding="utf-8").lower()
    evidence = Path("docs/cross_asset_ohlcv_evidence_plan.md").read_text(encoding="utf-8").lower()
    assert "cross-asset ohlcv preflight" in preflight
    ...
    assert "cross-asset ohlcv evidence plan" in evidence
    assert "row counts by `asset_group`" in evidence
    assert "row counts by `symbol`" in evidence
    assert "missing asset groups" in evidence
    assert "missing symbol coverage" in evidence
    assert "missing ohlc values" in evidence
    assert "stale `market_date` checks" in evidence
