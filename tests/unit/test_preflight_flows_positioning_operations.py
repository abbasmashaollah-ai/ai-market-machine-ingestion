...
def test_preflight_pass():
    import scripts.preflight_flows_positioning_operations as mod
    ...
    assert "expected_record_types_configured=True" in printed
    assert "expected_asset_classes_configured=True" in printed
...
def test_no_forbidden_imports():
    text = Path("scripts/preflight_flows_positioning_operations.py").read_text(encoding="utf-8")
    lowered = text.lower()
...
def test_docs_coverage():
    preflight = Path("docs/flows_positioning_preflight.md").read_text(encoding="utf-8").lower()
    evidence = Path("docs/flows_positioning_evidence_plan.md").read_text(encoding="utf-8").lower()
    assert "flows/positioning preflight" in preflight
    ...
    assert "flows/positioning evidence plan" in evidence
    assert "row counts by `record_type`" in evidence
    assert "row counts by `symbol`" in evidence
    assert "row counts by `asset_class`" in evidence
    assert "missing record types" in evidence
    assert "missing symbol coverage" in evidence
    assert "missing value/unit count" in evidence
