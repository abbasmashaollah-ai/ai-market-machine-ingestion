...
def test_preflight_pass():
    import scripts.preflight_options_operations as mod
    ...
    assert "expected_option_types_configured=True" in printed
    assert "expected_underlyings_configured=True" in printed
...
def test_no_forbidden_imports():
    text = Path("scripts/preflight_options_operations.py").read_text(encoding="utf-8")
    lowered = text.lower()
...
def test_docs_coverage():
    preflight = Path("docs/options_preflight.md").read_text(encoding="utf-8").lower()
    evidence = Path("docs/options_evidence_plan.md").read_text(encoding="utf-8").lower()
    assert "options preflight" in preflight
    ...
    assert "options evidence plan" in evidence
    assert "row counts by `underlying_symbol`" in evidence
    assert "row counts by `option_type`" in evidence
    assert "row counts by `expiration_date`" in evidence
    assert "missing contract coverage" in evidence
    assert "missing bid/ask/last counts" in evidence
    assert "missing volume/open_interest counts" in evidence
    assert "stale expiration metadata checks" in evidence
