...
def test_source_plan_shape():
    ...
    plans = build_news_sentiment_source_candidates()
    assert [plan.source_name for plan in plans] == ["Finnhub", "FMP", "Polygon", "manual_fixture"]
...
def test_source_plan_includes_required_candidates():
    ...
    names = [plan.source_name for plan in build_news_sentiment_source_candidates()]
...
def test_manual_fixture_marked_test_only():
    ...
    manual = [plan for plan in build_news_sentiment_source_candidates() if plan.source_name == "manual_fixture"][0]
    assert manual.status == "test_only"


def test_command_output():
    import scripts.plan_news_sentiment_sources as cmd
    ...
    assert "supported_record_shapes" in printed
    assert "next_required_step" in printed


def test_no_forbidden_imports():
    text = Path("scripts/plan_news_sentiment_sources.py").read_text(encoding="utf-8")
...
def test_docs_cover_source_plan():
    text = Path("docs/news_sentiment_source_plan.md").read_text(encoding="utf-8").lower()
    assert "news/sentiment source plan" in text
...
def test_symbol_master_dependency_documented():
    text = Path("docs/news_sentiment_vertical_slice_plan.md").read_text(encoding="utf-8").lower()
    assert "symbol_master" in text
    assert "ticker mapping" in text
