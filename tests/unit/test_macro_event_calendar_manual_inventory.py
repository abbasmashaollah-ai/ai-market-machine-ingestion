...
def test_manual_ingestion_command_inventory_mentions_macro_event_calendar_dry_run() -> None:
    text = Path("docs/manual_ingestion_command_verification.md").read_text(encoding="utf-8").lower()
    assert "scripts.dry_run_macro_event_calendar" in text
    assert "macro event calendar" in text
    assert "no vendor calls" in text
    assert "no database writes" in text


def test_verify_manual_ingestion_commands_mentions_macro_event_calendar_module() -> None:
