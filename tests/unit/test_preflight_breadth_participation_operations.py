...
def test_preflight_pass():
    import scripts.preflight_breadth_participation_operations as mod
    ...
    assert "expected_metric_types_configured=True" in printed
    assert "expected_universes_configured=True" in printed
...
def test_no_forbidden_imports():
    text = Path("scripts/preflight_breadth_participation_operations.py").read_text(encoding="utf-8")
    lowered = text.lower()
...
def test_docs_coverage():
    preflight = Path("docs/breadth_participation_preflight.md").read_text(encoding="utf-8").lower()
    evidence = Path("docs/breadth_participation_evidence_plan.md").read_text(encoding="utf-8").lower()
    assert "breadth/participation preflight" in preflight
    ...
    assert "breadth/participation evidence plan" in evidence
    assert "row counts by `metric_type`" in evidence
    assert "row counts by `universe`" in evidence
    assert "missing metric types" in evidence
    assert "missing universe coverage" in evidence
    assert "missing numerator/denominator count" in evidence
    assert "stale `observation_date` checks" in evidence
