# Polygon Stock Day Agg Ingestion Operator Approval Package
...
## Required Evidence Before Gates

Before any gate can be considered, the following evidence must exist:
...
These approval phrases are reserved for later operator use and are not active yet:
...
Each approval stage is intentionally narrow:
...
If a run fails, stop immediately on any vendor, auth, download, hash, parse, or validation mismatch.

Do not retry blindly.

Preserve logs and local evidence.

If rollback is approved, remove only local generated artifacts tied to the failed approved run.
...
Vendor credential names may be referenced by name only, but secret values must not be printed.

## Next Step

If a later operator record is filled and approved, proceed one gate at a time.
