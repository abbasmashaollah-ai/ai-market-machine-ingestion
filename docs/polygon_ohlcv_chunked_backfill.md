...
It fails safely before checkpoint resume if `DATABASE_URL` is missing and `--resume-from-checkpoint` is supplied.
...
It also fails safely if the supplied start date is after the end date.
...
When a chunk reports a sanitized 429 or rate-limit failure, the runner can stop the remaining chunks immediately, or continue until the configured `--max-rate-limit-failures` threshold is reached. Any unexpected chunk failure stops the remaining chunks for the run safely and reports the failed chunk. Sleeping between chunks is enabled by default and can be disabled for tests with `--dry-run-no-sleep`.

If `--resume-from-checkpoint` is supplied, the runner loads existing checkpoint state and skips completed chunk ranges without writing. If `--confirm-write` is also supplied, the runner updates checkpoint state after successful writes.
